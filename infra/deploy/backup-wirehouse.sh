...
docker compose -p wirehouse -f compose.yml run --rm --no-deps -T --entrypoint tar api -czf - -C /data . > "$target/uploads.tar.gz"
cp .env "$target/deployment.env"
cp compose.yml "$target/compose.yml"
if [[ -f compose.network.yml ]]; then cp compose.network.yml "$target/compose.network.yml"; fi
